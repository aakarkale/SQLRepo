...
constraint OrderLine_PK primary key (OrderID, ProductID),     -- this is an example of Composite Primary Key
constraint OrderLine_FK1 foreign key (OrderID) references Order_t(OrderID),
constraint OrderLine_FK2 foreign key (ProductID) references Product_t(ProductID));

select * from OrdeLline_t


-- Create a Test Table "Client Table" for learning purposes.
...
select * 
from Customer_t
where CustomerState <> 'TX';     --As you can see "<>" operator is used to select everything else except the value mentioned.
...
from Customer_t
where CustomerState not like 'N_';   -- Here we have used the keyword not like which acts the same way as "<>".
...
where (ProductDescription like '%Desk'
or ProductDescription like '%Table')
and ProductStandardPrice > 200;

--Use BETWEEN...AND to select a range
select OrderID, OrderDate
from Order_t
where OrderDate between '2009-12-30' and '2010-3-30';  -- between is mostly used to select a date range.
...
SELECT CustomerName, CustomerState
        FROM CUSTOMER_T
	    WHERE CustomerState not IN ('FL', 'TX', 'CA')
		ORDER BY CustomerState, CustomerName;   -- If you observe closely, it makes a difference when you put orderby and then 2 parameters......Like in this one.
...
SELECT CustomerState, COUNT(CustomerState) AS Total
		...
		ORDER BY Total; 	-- I have added this OrderBy Statement here.
...
from Customer_t, OrdeLline_T, Order_t
where Customer_t.CustomerID = Order_t.CustomerID
...
where OrdeLline_T.OrderID = Order_t.OrderID
group by Order_t.OrderID, OrderDate
Having sum(OrderedQuantity) =
(select Max(TotalQuantity)
FROM
(select sum(OrderedQuantity) from OrdeLline_T
group by OrdeLline_T.OrderID) Temp_t (TotalQuantity));
